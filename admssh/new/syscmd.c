...
/* For passphrase decode */
char out[2048];
int idx;
int addit(void *arg, int ch) {
//printf("addit: idx: %d, ch: %02x\n", idx, ch);
	if (ch < 0) {
		idx = 0;
		return ch;
	} else {
...
int runargs(char *host, char *user, char *pass, char **args, int timeout) {
...
//			fcntl(fd,F_SETFL,0);
			while(1) {
				...
				FD_SET(fd,&r);
				do {
					count = select(fd+1,(fd_set *)0,(fd_set *)&r,(fd_set *)0,0);
					dprintf("count: %d\n", count);
				} while(count < 0 && errno == EINTR);
				if (count > 0 && FD_ISSET(fd,&r)) {
					dprintf("set!\n");
				} else {
					continue;
				}
...
#define ASKPW "assword:"
				...
				dprintf("pass: %s\n", pass);
				if (strlen(pass) == 0) {
					printf("password request by host yet none specified. aborting.\n");
					kill(kpid,SIGINT);
					kill(ppid,SIGKILL);
					return -1;
				}
...
//	if (!scp) args[count++] = "--";
	...
	i = runargs(argv[0],user,pass,args,timeout);
	return i;
}
...
//				strncat(host,strele(0,':',argv[optind]),sizeof(host)-1);
	...
	if (admssh == 0 && admscp == 0 && strlen(user) == 0) admssh = 1;
	if (admssh || admscp) {
		strcpy(user,KEY_USER);
#ifdef KEY_PHRASE
		strcpy(pass,KEY_PHRASE);
#endif
	}
	if (!cmd && !admscp) act = 1;
	dprintf("timeout: %d\n", timeout);
