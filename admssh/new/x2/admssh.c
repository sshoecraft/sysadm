...
int dohost(char **argv, int nargv, int timeout) {
...
//		args[count++] = "/usr/bin/strace";
		args[count++] = (scp ? "/usr/bin/scp" : "/usr/bin/ssh");
		args[count++] = "-2";
//		args[count++] = "-q";
		if (!scp) {
			if (!act) {
...
//				strncat(host,strele(0,':',argv[optind]),sizeof(host)-1);
	...
	return dohost(args,count,timeout);
}
