...
//	char devs[65536], *p;
	...
	if (!devs) {
		err = GetLastError();
		printf("error allocating devs buffer: errno: %d(%x)\n", err, err);
		return lp;
	}
	dprintf("devs: %p\n", devs);

	sz = QueryDosDevice(0, devs, BUFLEN);
	dprintf("sz: %d\n", sz);
	err = GetLastError();
	if (!sz) {
		printf("errno: %d(%x)\n", err, err);
	...
	for(p = devs; *p; p += strlen(p)+1) {
		if (strncmp(p,PD,strlen(PD)) == 0) {
...
#if 0
			{
				register char *p2;
				char logical[512];
				sz = QueryDosDevice(p, logical, sizeof(logical));
				dprintf("sz: %d\n", sz);
				err = GetLastError();
				if (!sz) {
					printf("errno: %d(%x)\n", err, err);
				...
				for(p2 = logical; *p2; p += strlen(p2)+1) {
					dprintf("logical: %s\n", p2);
					break;
				}
			}
