...
void usage(void) {
	...
	printf("    -o                      put 2 spaces at the beginning of each line for outlook\n");
...
int main(int argc, char **argv) {
	...
	char *args[32];
	pid_t pid;
	int state,count,fdout,optind,have_bmark,nohead,nosub,add_status,onerr,outlook;

	prog = log = bmark = emark = email = from = sub = cc = bcc = 0;
	nosub = nohead = add_status = onerr = outlook = 0;
	fdout = 0;
