...
list get_vmware_devs(list lp) {
	...
	while((ent = readdir(dirp)) != 0) {
		...
		if (strncmp(ent->d_name,"naa",3) != 0 || strstr(ent->d_name,":")) continue;
		sprintf(name,"%s/",path);
