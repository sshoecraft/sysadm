...
char *get_tmpfile(char *template) {
...
#define SEP '/'
	...
	dprintf("tempdir: %s\n", p);
	sprintf(tmpfile,"%s%c%s",p,SEP,template);
	return tmpfile;
}

list get_mp_devs_from_name(list mpdevs,char *mp_name) {
...
int is_mp(list mpdevs, char *mp_name) {
	struct mpdevs_entry *mp;
	int r;
